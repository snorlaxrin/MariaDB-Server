...
#define FL (NOT_NULL_FLAG | NO_DEFAULT_VALUE_FLAG)
  return {{{"next_not_cached_value", len, handler, empty, FL},
           ...
           {"cache_size", 21, &type_handler_ulonglong, empty,
            FL | UNSIGNED_FLAG},
           {"cycle_option", 1, &type_handler_utiny, {STRING_WITH_LEN(
                                                                     "0 if no cycles are allowed, 1 if the sequence should begin "
                                                                     "a new cycle when maximum_value is passed")},
            FL | UNSIGNED_FLAG},
...
bool sequence_definition::check_sequence_fields(LEX *lex, List<Create_field> *fields)
{
  Create_field *field;
...
bool sequence_definition::prepare_sequence_fields(THD *thd, List<Create_field> *fields)
{
